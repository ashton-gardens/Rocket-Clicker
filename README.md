...
<head>
    ...
    <style>
        ...
        .price-tag { background: var(--gold-bg); padding: 2px 5px; border-radius: 4px; font-weight: 900; font-size: 10px; }
        ...
        #rocket { 
            font-size: 80px; /* Smaller Rocket */
            cursor: pointer; 
            transition: transform 0.05s; 
            z-index: 10; 
            filter: drop-shadow(0 0 10px rgba(0,210,255,0.3));
        }
        #rocket:active { transform: scale(0.9) translateY(-10px); }

        .satellite {
            position: absolute; 
            font-size: 22px; /* Smaller Satellites */
            pointer-events: none; 
            left: 50%; 
            top: 50%; 
            margin-left: -11px; 
        ...
        @keyframes orbit {
            /* Orbit distance adjusted for smaller rocket */
        ...
        #prestige-btn { display: none; background: var(--prestige-purple); color: white; border: none; padding: 8px; border-radius: 8px; font-weight: bold; width: 100%; margin-top: 5px; cursor: pointer; }
...
<div id="boss-comet" style="position:absolute; display:none; font-size:40px; z-index:90; cursor:pointer;">☄️</div>
...
<div class="sidebar">
    ...
    <div class="stats">
        ...
        <div style="color:white; font-size: 9px; font-weight: bold;">KILOMETERS</div>
    ...
    <div id="ach" class="tab-content">
        <div class="ach-item"><span>1k reached</span><span id="ach-1">🔒</span></div>
    </div>
...
<script>
    ...
    let upgrades = [
        { id: 'click1', name: 'Ion Thrusters', desc: '+2 click', cost: 50, power: 2, type: 'click' },
        { id: 'auto1', name: 'Satellites', desc: 'Auto Click', cost: 100, power: 5, type: 'auto', visual: '🛰️' },
        { id: 'auto2', name: 'Station', desc: '+100/s', cost: 5000, power: 100, type: 'auto' }
    ];
    ...
    function spawnVisualSatellite(emoji) {
        ...
        sat.innerText = emoji;
        // Reduced orbit distance (70-100px) to match smaller rocket
        sat.style.setProperty('--dist', (75 + Math.random() * 30) + 'px');
    ...
    function saveGame() {
        ...
        localStorage.setItem('rocketClickerV5', JSON.stringify(data));
        const ind = document.getElementById('save-indicator');
        ind.style.opacity = "1"; setTimeout(() => ind.style.opacity = "0", 1500);
    }

    function loadGame() {
        const saved = localStorage.getItem('rocketClickerV5');
        if (saved) {
